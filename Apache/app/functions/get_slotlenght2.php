...
if ($bedq){
    $row=$bedq->fetch_row();
    $beds=$row[0];
    $result1 = mysqli_query($connection,"select * from reservations where end>=".($end+1)." and room=".$room." order by start limit 1;");
    if ($result1){
        $row=$result1->fetch_row();
                $res->start=$row[2];
                if ($res->start == 0) $res->start = strtotime('+1 month', $start);
