...
function rendertheday($year,$month,$day, $reservations,$rooms,$roomnumbers,$yearly,$fakerender){
    ...
    if (($status &4) != 0) {
        ...
        if (($status&32) == 32 ) $co='danger';
        if (($status&16) == 16 ) $co='danger';
        if (($status&8) == 8 ) $co='danger';
        if (($status&256) == 256 ) $ci='danger';
        if (($status&128) == 128 ) $ci='danger';
        ...
        if ( ($status & 7)== 7){ //full all day show the room
            ...
            switch ($status & 7){
                case 1:{  // only check out today show checkout or book checkin
                    ...
                    $ret.= '<div class="progress-bar bg-'.$co.'" data-toggle="tooltip" title="" data-original-title="'.$textco.'" data-placement="top" data-container="body" style="width:50%; line-height:'.$height.'px;font-size:'.($height-2).'px;text-align:left;text-align:left;padding-left:5px" onclick="show_bookning(this)" data-bookningnr="'.$idout.'">'.$roomt.'</div> ';
                    $ret.= '<div class="progress-bar bg-success" style="width:0%"></div> ';
                    $ret.= '<div class="progress-bar bg-'.$ci.'" data-toggle="tooltip" title="" data-original-title="'.$textci.'" data-placement="top" data-container="body" style="width:50%" onclick="book(this)" data-bookningnr="'.$du.'" data-roomnr="'.$roomt.'"></div> ';
                    $ret.= '</div>';
                    break;
                }
                case 4:{ //only check in today show checkin do nothing for checkout
                    $ret= '<div class="progress" style="margin-bottom:1px; height:'.$height.'px; border-radius:0px">';
                    $ret.= '<div class="progress-bar bg-'.$co.'" data-toggle="tooltip" title="" data-original-title="'.$textco.'" data-placement="top" data-container="body" style="width:50%; line-height:'.$height.'px;font-size:'.($height-2).'px;text-align:left;text-align:left;padding-left:5px">'.$roomt.'</div> ';
                    $ret.= '<div class="progress-bar bg-success" style="width:0%"></div> ';
                    $ret.= '<div class="progress-bar bg-'.$ci.'" data-toggle="tooltip" title="" data-original-title="'.$textci.'" data-placement="top" data-container="body" style="width:50%" onclick="show_bookning(this)" data-bookningnr="'.$idin.'"></div> ';
                    $ret.= '</div>';
                    break;
                }
