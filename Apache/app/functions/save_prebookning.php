...
?>
    ...
    $timestamp=mktime();
    if (isset ($nr))   
        $command="UPDATE prebooknings SET room='".$room." ',start='".$checkintime." ',end='".$checkouttime." ',persons='".$persons." ',customer='".$name." ',email='".$email." ',status='".$kind." ',tel='".$tel." ',userremark='".$remark." ',user='".$user."',stamp='".$timestamp."',ip='".$ip."' WHERE idreservations= '".$nr."'";
    else{
        $command="INSERT INTO prebooknings (room,start,end,persons,customer,email,status,tel,userremark,user,stamp,ip) ";
